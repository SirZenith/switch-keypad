...
- `ML(x)`，momentary layer 在被按下时，层 x 会被激活；
- `TL(x)`，toggle layer， 被按下时，层 x 如果是非激活状态，则层 x 会被激活；反之层 x 会变为非激活。
- `OSL(x)`，one shot layer，被按下后，层 x 保持激活直到下一个按键被按下。在新按键被按下后，层 x 变为非激活。
